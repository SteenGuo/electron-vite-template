...
const { platform, release, arch } = window.system;
...
let tips = ref(
  computed(() => [
    ...
    { name: i18nt("about.currentPageName"), value: name },
    {
      ...
      value: process.env.NODE_ENV === "development" ? version : "不可见",
    },
    {
      name: i18nt("about.electronVersion"),
      value: window.versions?.electron || "浏览器环境",
    },
    {
      name: i18nt("about.nodeVersion"),
      value: window.versions?.node || "浏览器环境",
    },
